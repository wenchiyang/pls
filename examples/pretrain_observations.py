...
def generate_pacman(num_imgs, ghost_distance, folder):
    ...
    generate_random_images_pacman(csv_file, img_folder, num_imgs, ghost_distance)
...
if __name__ == "__main__":
    folder = "pacman_small_distance2"
    dir_path = os.path.dirname(os.path.realpath(__file__))
    model_folder = os.path.join(dir_path, "../experiments5/pacman/small3/data/")
    generate_pacman(num_imgs=5100, ghost_distance=2, folder=folder)
    # pre_train_gf(n_train=3000, net_class=Observation_Net_Stars, downsampling_size=8, folder=folder, model_folder=model_folder, epochs=201)
