...
class Pacman_DPLActorCriticPolicy(ActorCriticPolicy):
    ...
    def evaluate_actions(
        ...
        _actions, values, log_prob, mass, (object_detect_probs, base_actions) = self.forward(obs)
        log_prob = mass.log_prob(actions)

        # safe_next = self.get_step_safety(
        #     mass.probs,
        #     object_detect_probs["ground_truth_ghost"],
        #     None
        # )
        # not_safe_next = 1 - safe_next


        return values, log_prob, mass.entropy()#, not_safe_next
